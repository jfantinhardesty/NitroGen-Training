...
class ModalityConfig(BaseModel):
    ...
    frame_spacing: int | None = (
        None  # how many frames to skip between each frame. If None, use action_per_chunk
    )
    ...
    def model_post_init(self, __context):
        ...
        assert self.action_shift >= 1, (
            "Frame shift must be at least 1 for correct action indexing"
        )
...
class CkptConfig(BaseModel):
    ...
    model_cfg: NitroGen_Config = Field(
        ...,
        description="Model configuration. This is a placeholder and should be replaced with the actual model config class.",
    )
    tokenizer_cfg: NitrogenTokenizerConfig = Field(
        ...,
        description="Tokenizer configuration. This is a placeholder and should be replaced with the actual tokenizer config class.",
    )
